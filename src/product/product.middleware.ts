...
function validateCreateInput(req: Request, res: Response, next: NextFunction) {
	...
	const decimalPlaces = (input.price.toString().split('.')[1] || '').length
	if (decimalPlaces > 2) {
...
function validatePublishmentIdParam(req: Request, res: Response, next: NextFunction) {
	const publishmentId = Number.parseInt(req.params.publishmentId)

	if (isNaN(publishmentId) || publishmentId <= 0) {
		return HttpResponse.BadRequest(res, 'ID de publicación inválido')
	}

	next()
}

function validateArticleTypeIdParam(req: Request, res: Response, next: NextFunction) {
	const articleTypeId = Number.parseInt(req.params.articleTypeId)

	if (isNaN(articleTypeId) || articleTypeId <= 0) {
		return HttpResponse.BadRequest(res, 'ID de tipo de artículo inválido')
	}

	next()
}

function validateKayakTypeIdParam(req: Request, res: Response, next: NextFunction) {
	const kayakTypeId = Number.parseInt(req.params.kayakTypeId)

	if (isNaN(kayakTypeId) || kayakTypeId <= 0) {
		return HttpResponse.BadRequest(res, 'ID de tipo de kayak inválido')
	}
