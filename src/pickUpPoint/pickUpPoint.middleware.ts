...
function sanitizePickUpPointInput(req: Request, res: Response, next: NextFunction) {
    req.body.sanitizedInput = {
        ...
        adressFloor: !isNaN(Number(req.body.adressFloor)) && req.body.adressFloor !== '' ? Number(req.body.adressFloor) : undefined,
        adressApartment: typeof req.body.adressApartment === 'string' ? req.body.adressApartment.trim().toUpperCase() : undefined,
        tower: !isNaN(Number(req.body.tower)) && req.body.tower !== '' ? Number(req.body.tower) : undefined,
        localty: typeof req.body.localty === 'string' ? req.body.localty.trim() : undefined
    }
...
function validateCreateInput(req: Request, res: Response, next: NextFunction) {
    ...
    if (input.adressApartment !== undefined) {
        ...
        // Validar formato de departamento (letra o número opcional)
        const apartmentRegex = /^[A-Z0-9\-]*$/
        if (!apartmentRegex.test(input.adressApartment)) {
            return HttpResponse.BadRequest(res, 'El departamento solo puede contener letras mayúsculas, números y guiones')
        }
    }
    ...
    if (!input.localty) {
        return HttpResponse.BadRequest(res, 'El código postal de la localidad es requerido')
    }
...
function validateUpdateInput(req: Request, res: Response, next: NextFunction) {
    ...
    const hasFields = Object.keys(input).some(key => input[key] !== undefined && input[key] !== null && input[key] !== '')
    if (!hasFields) {
    ...
    if (input.localty !== undefined) {
        if (!input.localty || input.localty.trim() === '') {
            return HttpResponse.BadRequest(res, 'El código postal de la localidad no puede estar vacío')
        }
        const zipCodeRegex = /^[0-9]{4,5}$/
        if (!zipCodeRegex.test(input.localty)) {
            return HttpResponse.BadRequest(res, 'El código postal debe contener entre 4 y 5 dígitos numéricos')
