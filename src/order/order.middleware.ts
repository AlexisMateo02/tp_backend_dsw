...
function validateCreateOrderInput(req: Request, res: Response, next: NextFunction) {
    ...
    // 🆕 VALIDACIÓN CRÍTICA: Debe tener shippingAddress O pickUpPointId, pero no ambos
    if (input.shippingAddress && input.pickUpPointId) {
        return HttpResponse.BadRequest(res, 'La orden no puede tener dirección de envío y punto de retiro simultáneamente')
    }
    if (!input.shippingAddress && !input.pickUpPointId) {
        return HttpResponse.BadRequest(res, 'La orden debe tener dirección de envío o punto de retiro')
    }

    // 🆕 Validar shippingAddress si está presente
    if (input.shippingAddress) {
        if (input.shippingAddress.length < 10) {
            return HttpResponse.BadRequest(res, 'La dirección de envío debe tener al menos 10 caracteres')
        }
        if (input.shippingAddress.length > 500) {
            return HttpResponse.BadRequest(res, 'La dirección de envío no puede exceder los 500 caracteres')
        }
    }
...
function validateUpdateOrderInput(req: Request, res: Response, next: NextFunction) {
    ...
    // Solo permitir actualización de status, notes y shippingAddress
    const allowedFields = ['status', 'notes', 'shippingAddress']
