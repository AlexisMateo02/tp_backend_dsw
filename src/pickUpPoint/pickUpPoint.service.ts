...
export async function updatePickUpPoint(id: number, pickUpPointData: PickUpPointUpdateData) {
    ...
    if (updateData.adressStreet || updateData.adressnumber || updateData.localty) {
        ...
        const finalApartment = updateData.adressApartment !== undefined ? updateData.adressApartment : pickUpPoint.adressApartment
...
export async function deletePickUpPoint(id: number) {
    ...
    if (publishmentCount > 0) {
        throw new Error(`No se puede eliminar el punto de recogida. Existen ${publishmentCount} publicación${publishmentCount > 1 ? 'es' : ''} asociadas`)
    }
